...
def configure_provider_api_key_interactive(
    ...
    if defn.allow_custom_base_url:
        current_multi_key = bool(current.multi_key_enabled) if current else False
        multi_key_enabled = click.confirm(
            "Enable random multi-key rotation? (api_key supports comma-separated keys)",
            default=current_multi_key,
        )
