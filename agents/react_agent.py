...
class CoPawAgent(ReActAgent):
    def __init__(
        ...
        # No vendor-specific fallback — if nothing is configured the
        # agent simply won't start (the user must set up a provider).
        ...
        if llm_cfg and llm_cfg.api_key:
            ...
            # Single env-var fallback: COPAW_API_KEY only.
            # OPENAI_API_KEY / DASHSCOPE_API_KEY are intentionally NOT read
            # here to avoid silently using a wrong provider's credentials.
            # Use COPAW_BASE_URL to point at any OpenAI-compatible endpoint,
            # and COPAW_MODEL to select the model name.
            api_key = os.getenv("COPAW_API_KEY", "")
            base_url = os.getenv("COPAW_BASE_URL", "https://api.openai.com/v1")
            model_name = os.getenv("COPAW_MODEL", "gpt-4o-mini")

            if api_key:
                logger.info(
                    "No active LLM provider configured — falling back to "
                    "COPAW_API_KEY env var (model=%s, base_url=%s). "
                    "Configure a provider via the UI to silence this message.",
                    model_name,
                    base_url,
                )
            else:
                raise RuntimeError(
                    "No LLM configured.  Fix one of the following:\n"
                    "  1. Add a provider via the UI (POST /api/providers) "
                    "and mark it active, or\n"
                    "  2. Set COPAW_API_KEY (+ optionally COPAW_BASE_URL, "
                    "COPAW_MODEL) in your environment / .env file."
                )

        # Force HTTP/1.1 — some local proxies (e.g. copilot-api) do not
        # support HTTP/2 and return 502 when httpx negotiates h2.
        import httpx as _httpx

        _http_client = _httpx.AsyncClient(
            transport=_httpx.AsyncHTTPTransport(http1=True, http2=False),
        )
