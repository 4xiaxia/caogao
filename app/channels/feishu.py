...
class FeishuChannel(BaseChannel):
    ...
    def _run_ws_forever(self) -> None:
        ...
        def _create_unverified_context(*args: object, **kwargs: object) -> ssl_module.SSLContext:
        ...
        for attempt in range(2):
            try:
                if attempt == 1:
                    logger.warning(
                        "feishu WebSocket SSL certificate verification failed "
                        "on first attempt — retrying with SSL verification "
                        "disabled (common in corporate proxy / firewall "
                        "environments)."
                    )
                    ssl_module.create_default_context = _create_unverified_context
                    ssl_module._create_unverified_context = (  # type: ignore[attr-defined]
                        _create_unverified_context
                    )
                logger.info(
                    "feishu WebSocket connecting (long connection)%s...",
                    " [SSL verify disabled]" if attempt == 1 else "",
                )
                self._ws_client.start()
                break  # start() returned normally
            except Exception as exc:
                err = str(exc)
                is_ssl_error = (
                    "CERTIFICATE_VERIFY_FAILED" in err
                    or "certificate verify failed" in err.lower()
                    or "ssl" in err.lower()
                )
                if is_ssl_error and attempt == 0:
                    logger.warning(
                        "feishu WebSocket SSL error: %s — will retry without "
                        "certificate verification",
                        err,
                    )
                    continue
                logger.exception("feishu WebSocket thread failed")
                # Restore original on hard failure (not SSL retry path)
